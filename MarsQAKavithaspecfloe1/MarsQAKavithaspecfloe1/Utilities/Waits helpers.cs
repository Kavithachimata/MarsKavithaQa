...
namespace MarsQAspecflow1.Utilities
{
        ...
        {
            public static void WaitToBeClickable(IWebDriver driver, int seconds, string locator,
                                                                              string locatorValue)
            {
            ...
            public static void WaitToExist(IWebDriver driver, int seconds, string locator,
                                                                                      string locatorValue)
            {
                var wait = new WebDriverWait(driver, new TimeSpan(0, 0, seconds));
                if (locator == "XPath")
                ...
                if (locator == "Id")
                {
                    wait.Until(SeleniumExtras.WaitHelpers.ExpectedConditions.ElementExists(By.Id
                                                                     (locatorValue)));
                }
                if (locator == "CssSelector")
                {
